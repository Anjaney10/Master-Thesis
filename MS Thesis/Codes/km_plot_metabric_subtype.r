...
# Code for METABRIC KM Plot - Subtype wise

########################### KM Plot for METABRIC data subtype wise ############################
library(survival)
library(survminer)
library(dplyr)
library(ggplot2)
...
output_dir <- "METABRIC_KM_Plots_Subtype_HR_Pval"
...
metabric_final_df_filtered <- metabric_final_df %>%
...
if (nrow(metabric_final_df_filtered) == 0) {
...
for (i in seq_along(signature_bases)) {
  ...
  # Data Prep
  if (!sig_level_col %in% colnames(metabric_final_df_filtered)) { cat(" Column not found. Skipping.\n"); next }
  df_signature_filtered <- metabric_final_df_filtered %>%
  ...
  for (subtype in subtypes_to_plot) {
    ...
    if (!is.null(ggsurv_obj)) {
         max_time <- max(df_subtype_signature[[time_col]], na.rm=TRUE) * 0.8
         y_pos = 0.95
  ...
  if(length(successful_plots) > 0) {
     ...
     final_plot_labeled <- final_plot_intermediate +
      ...
      filename <- file.path(output_dir, paste0("METABRIC_KM_Plot_", sig_base, "_Final_HR_Pval.png"))
