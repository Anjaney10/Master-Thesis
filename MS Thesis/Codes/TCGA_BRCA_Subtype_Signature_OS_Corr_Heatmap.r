...
# Code for TCGA_BRCA_Subtype_Signature_OS_Corr_Heatmap
...
output_heatmap_file <- file.path(output_dir, "TCGA_BRCA_Subtype_Signature_OS_Corr_Heatmap.png")

missing_req_cols <- setdiff(required_cols, colnames(tcga_final_df))
if (length(missing_req_cols) > 0) {
    stop("Error: The following required columns are missing in 'tcga_final_df
  ': ",
         paste(missing_req_cols, collapse=", "))
}

data_for_corr <- tcga_final_df %>%
  filter(
    !is.na(.data[[subtype_col]]) & .data[[subtype_col]] != "" & .data[[subtype_col]] != "Normal",
...
subtype_counts <- data_for_corr %>% count(.data[[subtype_col]])
print("Data counts per subtype after initial filtering:")
...
heatmap_plot <- ggplot(correlation_heatmap_data, aes(y = .data[[subtype_col]], x = Signature, fill = Correlation)) +
  ...
  labs(
    title = "TCGA BRCA Correlation: Signature ssGSEA vs OS Time by BRCA Subtype",
  )
